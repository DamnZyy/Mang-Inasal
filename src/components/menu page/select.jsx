...
import FoodItem, { FoodItemContainer } from './foods'

export default function SimpleSlider({choices, FoodDetails}) {
  ...
  return (
    <div className="">
      ...
      <FoodItemContainer>
          {FoodDetails[Option] && FoodDetails[Option].map((foods, index) => <FoodItem href={`delivery/${foods.id}`} key={index} image={foods.image} title={foods.title} description={foods.description}></FoodItem>)}
      </FoodItemContainer>
    </div>
  );
}